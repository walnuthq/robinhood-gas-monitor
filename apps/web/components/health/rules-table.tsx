...
import { RULES, SEVERITY } from "@/lib/health/rules"
import type { HealthSnapshot } from "@/lib/health/types"

/**
 * The rules, and how each did: when it first fired in the incident replay, how
 * far ahead of the reported halt, and how often it fired outside the incident.
 * The last column is what separates an alarm from noise. It counts episodes, not
 * days, so a false alarm on the incident's own date (Sep 4, 09:20) still shows.
 */
          ...
          <TableBody>
            {RULES.map((rule) => {
              ...
              const onIncident = incident
                ? own
                    .filter(
                      (a) => a.start >= incident.from && a.start <= incident.to
                    )
                    .sort((a, b) => a.start - b.start)[0]
                : undefined
              const outside = own.filter(
                (a) =>
                  a.start < incidentPeriod.from || a.start > incidentPeriod.to
              )
              const outsideDays = new Set(outside.map((a) => formatDay(a.start)))
              ...
              return (
                <TableRow key={rule.id}>
                  ...
                  <TableCell className="text-right font-mono text-xs tabular-nums">
                    {onIncident ? formatClock(onIncident.start) : "—"}
                  </TableCell>
